package org.gra.poi.bl;

import java.io.Serializable;
import java.util.List;
import org.springframework.beans.factory.annotation.*;
import org.springframework.stereotype.*;
import org.gra.poi.be.*;
import org.gra.poi.da.*;
import org.gra.poi.utl.*;

@Service
public class AsignacionAeOeiBL  extends AbstractBL<AsignacionAeOei> implements Serializable{

    @Autowired
    @Qualifier("asignacionAeOeiDA")
    private AsignacionAeOeiDA dao;
    
    @Override
    public AbstractDA getDAO() {
        return dao;
    }

    @Override
    public void setDA(AbstractDA dao) {
        this.dao = (AsignacionAeOeiDA) dao;
    }

    @Override
    public long registrar(AsignacionAeOei bean) {
         return save(bean);
    }

    @Override
    public long actualizar(AsignacionAeOei bean) {
         return update(bean);
    }

    @Override
    public long eliminar(AsignacionAeOei bean) {
         return delete(bean);
    }

    @Override
    public List<AsignacionAeOei> listar() {
         return list();
    }

    @Override
    public List<AsignacionAeOei> listar(String ref) {
        return list(ref);
    }

    @Override
    public List<AsignacionAeOei> listar(long id) {
        return list(id);
    }

    @Override
    public AsignacionAeOei buscar(long id) {
        return search(id);
    }

    @Override
    public AsignacionAeOei buscar(String ref) {
        return search(ref);
    }

    @Override
    public long id() {
        return maxId();
    }
    
}

